...
"""
    ...
    outstr += basic_size_offset.format(tracefile, tracefile, tracefile,
                                       (lastTime - firstTime) /
                                       1000, convertToByteStr(
                                           write_max_size * 512),
                                       ioCount, convertToByteStr(
                                           write_min_size * 512),
                                       float(
                                           ioCount) / totalsec, convertToByteStr(read_max_size * 512),
                                       float(writeCount) / float(ioCount) *
                                       100, convertToByteStr(
                                           read_min_size * 512),
                                       float(total_write_size) / totalsec,
                                       float(
                                           writeCount) / totalsec, convertToByteStr(write_max_offset * 512),
                                       float(
                                           total_write_size) / writeCount, convertToByteStr(write_min_offset*512),
                                       float(readCount) / float(ioCount) * 100, convertToByteStr(
                                           (write_max_offset - write_min_offset)*512),
                                       float(
                                           total_read_size) / totalsec, convertToByteStr(read_max_offset * 512),
                                       float(
                                           readCount) / totalsec, convertToByteStr(read_min_offset*512),
...
"""
    ...
    outstr += sizebucket.format(str(write_countbucket), sum(write_countbucket),
                                ...
                                str_write_sizebucket, convertToByteStr(
                                    str(sum(write_sizebucket))),
...
"""
    ...
    outstr += plot.format(str(writeSize[0]) + ", " + str(writeSize[int((writeCount-1)/4)]) + ", " + str(writeSize[int((writeCount-1)/2)]) + ", "+str(writeSize[int(3*(writeCount-1)/4)])+", "+str(writeSize[writeCount-1]),
                          str(readSize[0]) + ", " + str(readSize[int((readCount-1)/4)]) + ", " + str(readSize[int(
                              (readCount-1)/2)]) + ", "+str(readSize[int(3*(readCount-1)/4)])+", "+str(readSize[readCount-1]),
                          "{:.02f}".format(timeInterval[0]) + ", " + "{0:.2f}".format(timeInterval[int((len(timeInterval)-1)/4)]) + ", " + "{0:.2f}".format(timeInterval[int(
                              (len(timeInterval)-1)/2)]) + ", "+"{0:.2f}".format(timeInterval[int(3*(len(timeInterval)-1)/4)])+", "+"{0:.2f}".format(timeInterval[len(timeInterval)-1]),
