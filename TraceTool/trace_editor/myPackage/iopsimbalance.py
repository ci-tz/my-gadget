    ...
    for i in range(0, len(inputdisk)):
        for request in inputdisk[i]:
            if (int(float(request[0]) * 1000) / (delta * 1000)) not in bucket:
                bucket[int(float(request[0]) * 1000) /
                       (delta * 1000)] = [0] * len(inputdisk)
            bucket[int(float(request[0]) * 1000) / (delta * 1000)][i] += 1

    for key in sorted(bucket):
        print(str(int(key * granularity)) + "-" + str(int(key * granularity + granularity)) + ": " +
              str(bucket[key]) + " - imbalance:" + str(float(max(bucket[key]) / median(bucket[key]))))
