...
def rresize(reqlist, times):
    for request in reqlist:
        if int(request[4]) == 1:
            request[3] = ('%f' % (times * float(request[3]))
                          ).rstrip('0').rstrip('.')
    return reqlist
...
def insertIO(reqlist, size, interval, iotype):
    ...
    while i < len(reqlist):
        if float(reqlist[i][0]) > insert_time:  # 7190528,7370752
            reqlist.insert(i, ['%.3f' % insert_time, str(0), str(
                randint(0, maxoffset)), str(size * 2), str(iotype)])
            insert_time += interval
            i += 1
        i += 1
    return reqlist
...
def printRequestList(requestlist, filename):
    ...
    for elm in requestlist:
        out.write(str(elm[0]) + " " + str(elm[1]) + " " +
                  str(elm[2]) + " " + str(elm[3]) + " " + str(elm[4])+"\n")
    out.close()
