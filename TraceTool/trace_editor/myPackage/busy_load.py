...
def checkCongestedTime(tracefile, process_type, devno, minutes, top=1):
        ...
        if int(tok[1]) == devno:
            if process_type == "3":
                if (int(float(tok[0]) * 1000)/timerange) not in result:
                    ...
                    # size count
                    result[int(float(tok[0]) * 1000) /
                           timerange][0] += (float(tok[3]) / 1024)
                    result[int(float(tok[0]) * 1000) /
                           timerange][1] += 1  # IOs count
            else:
                if (int(float(tok[0]) * 1000)/timerange) not in result:
    ...
    for elm in sorted(list(result.items()), key=operator.itemgetter(1), reverse=True):
        print("time(minutes): " + str(elm[0] * minutes) + "-" + str(
            elm[0] * minutes + minutes) + ": " + str(elm[1]).rstrip('0').rstrip('.') + " " + unit)
        i += 1
        if i >= top:
            break
