...
def sanitize(tracefile, maxsize):
    ...
    with open("in/" + tracefile, "r") as myfile:
        tracein = myfile.read()
        out.write(fitIOToDisk(removeRepeatedReads(
            fuseContiguousIO(tracein)), maxsize))  # 2147483648
...
def fuseContiguousIO(tracein):
    ...
    for line in tracein.splitlines():
        # time,devno,blkno,blkcount,flag-0 write/1 read
...
def removeRepeatedReads(tracein):
    ...
    for line in tracein.splitlines():
        ...
        if (tok[4] == "0") or (0 in has_been_read[int(tok[2]):int(tok[2]) + int(tok[3])]):
            ...
            if (tok[4] == "1"):
                has_been_read[int(tok[2]):int(tok[2]) +
                              int(tok[3])] = int(tok[3]) * bitarray([True])
